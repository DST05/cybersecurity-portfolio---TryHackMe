...
- **Hypervisor** – Software that creates and manages virtual machines while allocating resources such as CPU, memory and storage.
- **Container** – An isolated environment used to run an application and its dependencies. Containers share the host operating system's kernel, making them lightweight and efficient.
...
The practical activity also helped me understand how virtual environments can be monitored and managed rather than only learning about them theoretically.

Virtualisation is important in cybersecurity because it allows security professionals to create isolated environments for testing, investigation and learning. For example, a virtual machine can be used to run Linux or cybersecurity tools without having to install them directly onto the main operating system. Understanding virtualisation will therefore be useful when I create my own cybersecurity lab and practise security techniques in a controlled environment.
